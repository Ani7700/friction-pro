...
import { TbCircleKeyFilled, TbAffiliateFilled, TbDownload } from "react-icons/tb";
import {
  RevisionItem,
  GlobalHumanPlanItem,
    ...
    <div className="flex flex-col h-screen w-full py-4 px-8 bg-gray-50 text-gray-800">
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-row gap-2 flex-wrap">
          ...
          <Button
            color="dark"
            className="w-48 enabled:hover:ring-gray-800 enabled:ring-gray-800"
            onClick={() => {
              if (essay.length === 0) {
                alert("No essay to export.");
                return;
              }
              const name = `essay-revised-${new Date().toISOString().slice(0, 10)}.txt`;
              downloadEssayAsFile(essay, name);
              eventTracker({
                event: "export_essay",
                sentence:
          ...
          >
            <TbDownload size={20} className="mr-2 inline" />
            Export essay
          </Button>
        </div>
